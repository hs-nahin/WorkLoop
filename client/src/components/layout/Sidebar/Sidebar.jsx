      ...
      <nav className="mt-8 px-4 space-y-2">
        {filteredItems.map((item) => (
          <Link 
            key={item.path} 
            to={item.path} 
            className={`flex items-center gap-4 p-3 rounded-xl transition-all duration-300 group ${location.pathname === item.path ? 'bg-yellow-400 text-black font-bold' : 'text-gray-400 hover:bg-white/10 hover:text-white'}`}
          >
